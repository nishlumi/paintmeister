...
//#################################################################################
	var Draw = {
		...
		resizeCanvasMargin : function (winwidth, winheight){
			...
			//document.getElementById("basepanel").style.left = (45 + space) + "px";
			console.log("left="+space + "/" + spacey);
			//ElementTransform(document.getElementById("canvaspanel"),"translate("+space+"px," + spacey + "px)");
		...
		scaleUp : function (){
			var fnlbi = 1.0;
			fnlbi = this.during_scale + 0.1;
			console.log("fnlbi="+ fnlbi);
			if (fnlbi > 4.0) {
				fnlbi = 4.0;
			}
		...
		touchMove : function(event) {
			...
			if (this.is_scaling) { //拡大縮小モード
				...
				var pow_x = Math.pow(this.scale_pos["begin"].x - (-1 * this.scale_pos["end"].x),2);
				var pow_y = Math.pow(this.scale_pos["begin"].y - (-1 * this.scale_pos["end"].y),2);
				var powsum = pow_x + pow_y;
